...
java ${JAVA_OPTS} -jar /opt/selenium/selenium-server.jar node \
  --publish-events tcp://selenium-hub:${SE_EVENT_BUS_PUBLISH_PORT} \
  --subscribe-events tcp://selenium-hub:${SE_EVENT_BUS_SUBSCRIBE_PORT} \
  --detect-drivers false \
  --log ${NODE_LOG_PATH} \
  --registration-secret ${SE_PREGISTRATIONKEY} \
